(function(){
  ...
  let pc = null, micStream = null, eventsDC = null;
  ...
  let autosaveTimer = null, dirty = false, lastSnapshot = "", lastSaveAt = 0, saving = false;
  let finalized = false;               // conversation closed on server
  let greetingPending = false;         // we sent/expect a closing message from the model
  let greetTimeoutHandle = null;       // fallback timer to stop if no response.completed arrives
  ...
  function sendClosingMessage(reason) {
    if (!eventsDC || eventsDC.readyState !== "open") return false;
    const text = "Thanks for spending your time with us. Your session is complete. Have a wonderful day!";
    try {
  ...
  function handleFunctionCallEvent(evt) {
    ...
    if (evt.type === "response.function_call.arguments.done" || evt.type === "response.tool_call.completed") {
      const buf = fnArgBuffers[id] || ""; delete fnArgBuffers[id];
      let args = {};
      try { const s = buf.indexOf("{"), e = buf.lastIndexOf("}"); const jsonStr = (s !== -1 && e > s) ? buf.slice(s, e+1) : buf; args = JSON.parse(jsonStr); } catch {}
